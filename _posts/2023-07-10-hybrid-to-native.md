...
3、如果这个内容包含图片、视频等内容，那么只展示一部分文字内容，然后再文本内容后面展示这个内容中的图片的缩略图，如下图所示
![折叠图片](/img/2023-07-10/2023-07-10-native-pic.jpg)
4、在知乎的监控系统上，知乎站内 ANR 每天差不多发生 3000+；经过分析绝大部分都是以渲染长文引起的，这个急需治理，而在当前 HTML 文本下基本无解；   
...
由于知乎的回答、文章的详情页是知乎核心的消费场景，知乎各个部门都有业务在详情页中流通；如商业部门的各种类型的广告卡片、好物卡片、芝士卡片等等；会员部门的促销卡片、会员购买卡片、盐选卡片等等，这些第三方卡片应该由他们自己解析和渲染，我们的详情页应该只提供槽位；同时也考虑到知乎有些文章、回答的内容比较长，直接使用一个 `UITextView` 渲染可能存在性能问题；我们决定采取 PDD 的这种方案来对 hybrid 详情页 native 化；那么如何把前端的标签和客户端的 UIView 建立映射，这个需要前端、客户端、内容平台、创作侧一起坐下来聊聊，如何建立它们之间的映射；知乎的内容详情页的结构化协议就这样诞生了😄；
...
**注：上面是大致介绍知乎是如何实现 hybrid 转 native 的；在实际实践过程中，肯定有很多怪要打 😁**